...
function getAMTIdsFromURL(url){
    return {
        "assignmentId": getParameterByName("assignmentId",url),
        "workerId": getParameterByName("workerId",url),
        "hitId": getParameterByName("hitId",url)
    }
}
...
function sendToBackground(operation, data, callback){
    chrome.runtime.sendMessage({ "operation": operation, "data": data }, function(data){ if(callback) callback(data); });
}

function sendToBackgroundPromise(operation, data, assignmentId){
    return new Promise(function(resolve,reject){
        if(assignmentId) var d = jQuery.extend(true, data, {"assignmentId": assignmentId});
        else var d = data;
        chrome.runtime.sendMessage({ "operation": operation, "data": d }, function(data){
            if(typeof(data.statusText)==="undefined") resolve(data);
            else reject(data);
        });
    });
}

// cannot be used from main script (available only in background)
function saveToDBPromise(path, data){
    return new Promise(function(resolve,reject){
        $.ajax({
            type: "post",
            datatype: "json",
